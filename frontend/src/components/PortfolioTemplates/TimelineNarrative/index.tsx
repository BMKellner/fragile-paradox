...
import styles from "./TimelineNarrative.module.css";
import {
  SectionType,
  sectionTitle,
...
export default function TimelineNarrativePortfolio({
  personalInformation,
  overviewData,
  projects,
  experience,
  ...
  const [timelineView, setTimelineView] = useState<"experience" | "education">("experience");
  ...
  const hasExperienceSection = useMemo(
    () => sections.some((section) => section.type === SectionType.Experience),
    [sections]
  );
  const hasEducationSection = useMemo(
    () => sections.some((section) => section.type === SectionType.Education),
    [sections]
  );
  ...
  useEffect(() => {
    if (timelineView === "experience" && !hasExperienceSection && hasEducationSection) {
      setTimelineView("education");
    }
    if (timelineView === "education" && !hasEducationSection && hasExperienceSection) {
      setTimelineView("experience");
    }
  }, [timelineView, hasExperienceSection, hasEducationSection]);
  ...
  const accent = sanitizeHexColor(resolvedConfig.theme.primaryColor, "#f59e0b");
  const bg = sanitizeHexColor(resolvedConfig.theme.backgroundColor, "#fffdf7");
      ...
      case SectionType.Hero: {
        ...
        return (
          <section key={section.id} id={section.id} className={styles.hero} data-reveal>
            ...
            <h2>{content.careerName || "Narrative Engineer"}</h2>
            <p className={styles.heroSummary}>{content.summary || "Building long-term impact through projects, teams, and consistent delivery over time."}</p>
        ...
        return (
          <section key={section.id} id={section.id} className={styles.section} data-reveal>
            <header className={styles.sectionHeader}>
              <h2>{content.title}</h2>
              <p>{content.subtitle}</p>
              {hasExperienceSection && hasEducationSection ? (
                <div className={styles.timelineControls}>
                  <button
                    type="button"
                    className={`${styles.timelineButton} ${
                      timelineView === "experience" ? styles.timelineButtonActive : ""
                    }`}
                    onClick={() => setTimelineView("experience")}
                  >
                    Experience
                  </button>
                  <button
                    type="button"
                    className={`${styles.timelineButton} ${
                      timelineView === "education" ? styles.timelineButtonActive : ""
                    }`}
            ...
            <div className={styles.timeline}>
              {timelineView !== "experience" ? (
                <article className={styles.card}>
                  <p>Timeline filter is showing Education. Switch tabs to view Experience entries.</p>
                </article>
              ) : content.items.length ? (
                content.items.map((item, index) => {
                  const entryId = `experience-${index}`;
                  const isOpen = expandedTimelineItem === entryId;
                  ...
                  return (
                    <article key={`${item.company}-${index}`} className={styles.timelineCard}>
                      <button
                        ...
                        onClick={() => setExpandedTimelineItem(isOpen ? null : entryId)}
                      ...
                      <div
                        className={`${styles.timelineDetails} ${
                          isOpen ? styles.timelineDetailsOpen : ""
                        }`}
                      >
                        <ul>
          ...
          <section key={section.id} id={section.id} className={styles.section} data-reveal>
            ...
            <div className={styles.stackList}>
              {timelineView !== "education" ? (
                <article className={styles.card}>
                  <p>Timeline filter is showing Experience. Switch tabs to view Education entries.</p>
                </article>
      ...
      <div className={styles.layout}>
        <aside className={styles.rail}>
          <div className={styles.railTop}>
            ...
            <p>{heroContent.careerName || "Narrative Engineer"}</p>
          </div>
